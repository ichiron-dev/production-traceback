<script lang="ts">
  ...
  const { data }: { data: AeoiPageData } = $props();
  ...
  function applyFilter(): void {
    ...
    goto(`/dashboard/data/aeoi?${params.toString()}`, { replaceState: true });
  }
  ...
  function resetFilter(): void {
    ...
    filterDateFrom = '';
    filterDateTo   = '';
    goto('/dashboard/data/aeoi', { replaceState: true });
  }

  // ── Columns ──────────────────────────────────────────────────────────────────
  const columns: ColDef<AeoiRecord>[] = [
    {
      key: 'id',
      header: 'ID',
    ...
    },
    {
      key: 'mc_no',
      header: 'MC No.',
      cell: (r) => `<span class="font-mono text-xs">${r.mc_no}</span>`,
      sortValue: (r) => r.mc_no,
    },
    {
      key: 'line',
    ...
    },
    {
      key: 'state',
      header: 'State',
      cell: (r) => `<span class="font-mono text-xs">${r.state}</span>`,
      sortValue: (r) => r.state,
    },
    {
      key: 'arp_tray',
    ...
    },
    {
      key: 'ep_center_x',
      header: 'EP Center X',
      cell: (r) => `<span class="font-mono text-xs">${r.ep_center_x.toFixed(4)}</span>`,
      sortValue: (r) => r.ep_center_x,
    },
    {
      key: 'ep_center_y',
      header: 'EP Center Y',
      cell: (r) => `<span class="font-mono text-xs">${r.ep_center_y.toFixed(4)}</span>`,
      sortValue: (r) => r.ep_center_y,
    },
    {
      key: 'ep_theta',
      header: 'EP Theta',
      cell: (r) => `<span class="font-mono text-xs">${r.ep_theta.toFixed(4)}</span>`,
      sortValue: (r) => r.ep_theta,
    },
    {
      key: 'tl_x',
      header: 'TL X',
      cell: (r) => `<span class="font-mono text-xs">${r.tl_x.toFixed(4)}</span>`,
      sortValue: (r) => r.tl_x,
    },
    {
      key: 'tl_y',
      header: 'TL Y',
      cell: (r) => `<span class="font-mono text-xs">${r.tl_y.toFixed(4)}</span>`,
      sortValue: (r) => r.tl_y,
    },
    {
      key: 'tr_x',
      header: 'TR X',
      cell: (r) => `<span class="font-mono text-xs">${r.tr_x.toFixed(4)}</span>`,
      sortValue: (r) => r.tr_x,
    },
    {
      key: 'tr_y',
      header: 'TR Y',
      cell: (r) => `<span class="font-mono text-xs">${r.tr_y.toFixed(4)}</span>`,
      sortValue: (r) => r.tr_y,
    },
    {
      key: 'bl_x',
      header: 'BL X',
      cell: (r) => `<span class="font-mono text-xs">${r.bl_x.toFixed(4)}</span>`,
      sortValue: (r) => r.bl_x,
    },
    {
      key: 'bl_y',
      header: 'BL Y',
      cell: (r) => `<span class="font-mono text-xs">${r.bl_y.toFixed(4)}</span>`,
      sortValue: (r) => r.bl_y,
    },
    {
      key: 'br_x',
      header: 'BR X',
      cell: (r) => `<span class="font-mono text-xs">${r.br_x.toFixed(4)}</span>`,
      sortValue: (r) => r.br_x,
    },
    {
      key: 'br_y',
      header: 'BR Y',
      cell: (r) => `<span class="font-mono text-xs">${r.br_y.toFixed(4)}</span>`,
      sortValue: (r) => r.br_y,
    },
    {
      key: 'cp_index',
      header: 'CP Index',
      cell: (r) => `<span class="font-mono text-xs">${r.cp_index}</span>`,
      sortValue: (r) => r.cp_index,
    },
    {
      key: 'cp_width',
      header: 'CP Width',
      cell: (r) => `<span class="font-mono text-xs">${r.cp_width.toFixed(4)}</span>`,
      sortValue: (r) => r.cp_width,
    },
    {
      key: 'cp_length',
      header: 'CP Length',
      cell: (r) => `<span class="font-mono text-xs">${r.cp_length.toFixed(4)}</span>`,
      sortValue: (r) => r.cp_length,
    },
    {
      key: 'judgement',
      header: 'Judgement',
      cell: (r) => {
        const ng = r.judgement === 'NG';
        return `<span class="text-xs font-bold px-2 py-0.5 rounded ${ng ? 'bg-[rgba(247,111,111,0.12)] text-[#f76f6f] border border-[rgba(247,111,111,0.3)]' : 'bg-[rgba(99,230,190,0.1)] text-[#63e6be] border border-[rgba(99,230,190,0.25)]'}">${r.judgement || '-'}</span>`;
      },
      sortValue: (r) => r.judgement,
    },
    {
      key: 'created_at',
      header: 'วันที่',
  ...
  function toExcelRows(items: AeoiRecord[]): ExcelRow[] {
    return items.map((r) => ({
      'ID':          r.id,
      'Lot':         r.lot,
      'Model':       r.model || '-',
      ...
      'ARP Tray':    r.arp_tray || '-',
      'Pos No.':     r.pos_no,
      'EP Center X': r.ep_center_x,
      'EP Center Y': r.ep_center_y,
      'EP Theta':    r.ep_theta,
      'TL X':        r.tl_x,
      'TL Y':        r.tl_y,
      'TR X':        r.tr_x,
...
<svelte:head><title>AEOI — Traceback</title></svelte:head>

<Topbar title="AEOI" breadcrumbs={[{ label: 'ข้อมูล' }, { label: 'AEOI' }]} />
...
<div class="flex-1 overflow-y-auto p-7 flex flex-col gap-6">
  ...
  {:else}
    <DataTable
      data={data.items}
      {columns}
      title="AEOI — {data.items.length} รายการ"
      exportFilename="aeoi_{new Date().toISOString().slice(0,10)}"
      exportSheetName="AEOI"
      {toExcelRows}
    />
  {/if}
</div>
