<script lang="ts">
  ...
  const { data }: { data: CaseSettingPageData } = $props();
  ...
  function applyFilter(): void {
    ...
    goto(`/dashboard/data/case-setting?${params.toString()}`, { replaceState: true });
  }
  ...
  function resetFilter(): void {
    ...
    filterDateFrom = '';
    filterDateTo   = '';
    goto('/dashboard/data/case-setting', { replaceState: true });
  }

  // ── Columns ──────────────────────────────────────────────────────────────────
  const columns: ColDef<CaseSettingRecord>[] = [
    {
      key: 'no',
      header: 'No.',
    ...
    },
    {
      key: 'case_setting',
      header: 'Case Setting',
      cell: (r) => r.case_setting
        ? `<span class="text-xs font-semibold bg-[rgba(99,230,190,0.08)] text-[#63e6be] px-2 py-0.5 rounded border border-[rgba(99,230,190,0.2)]">${r.case_setting}</span>`
        : `<span class="text-(--color-muted)">-</span>`,
      sortValue: (r) => r.case_setting,
    },
    {
      key: 'arp_tray',
      header: 'ARP Tray',
  ...
  function toExcelRows(items: CaseSettingRecord[]): ExcelRow[] {
    return items.map((r) => ({
      'No':           r.no,
      'Case Setting': r.case_setting,
      'ARP Tray':     r.arp_tray || '-',
      'Empcode':      r.empcode,
      'Line':         r.line || '-',
      'Lot':          r.lot,
...
<svelte:head><title>Case Setting — Traceback</title></svelte:head>

<Topbar title="Case Setting" breadcrumbs={[{ label: 'ข้อมูล' }, { label: 'Case Setting' }]} />
...
<div class="flex-1 overflow-y-auto p-7 flex flex-col gap-6">
  ...
  {:else}
    <DataTable
      data={data.items}
      {columns}
      title="Case Setting — {data.items.length} รายการ"
      exportFilename="case_setting_{new Date().toISOString().slice(0,10)}"
      exportSheetName="CaseSetting"
      {toExcelRows}
    />
  {/if}
</div>
